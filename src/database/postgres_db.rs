...
pub struct PostgresDatabase {
    /// Database URI.
    pub uri: String,
    /// A  client object to the postgres database
    pub client: RefCell<Client>,
}

fn u64_to_i64(value: u64) -> Result<i64, Error> {
    i64::try_from(value).map_err(|e| Error::Generic(format!("{e}")))
}

fn u32_to_i32(value: u32) -> Result<i32, Error> {
    i32::try_from(value).map_err(|e| Error::Generic(format!("{e}")))
}

fn optu64_to_opti64(opt: Option<u64>) -> Result<Option<i64>, Error> {
    if let Some(value) = opt {
        return Ok(Some(u64_to_i64(value)?));
    }
...
impl TryFrom<&TransactionDetails> for TransactionDetailsInternal {
    ...
    fn try_from(value: &TransactionDetails) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            txid: value.txid,
            received: u64_to_i64(value.received)?,
            sent: u64_to_i64(value.sent)?,
            fee: optu64_to_opti64(value.fee)?,
...
impl TryFrom<&BlockTime> for BlockTimeInternal {
    ...
    fn try_from(value: &BlockTime) -> Result<Self, Self::Error> {
        Ok(Self {
            height: u32_to_i32(value.height)?,
            timestamp: u64_to_i64(value.timestamp)?,
        })
    }
}

// TODO: should return mutable client through `Client::connect`.
// That in turn implements all the trait methods, and PostgresDatabase
// would be just a thin wrapper.

impl PostgresDatabase {
    /// Instantiate a new Postgres Database instance by creating a client
    /// passing it the remote database URI.
    pub fn new(uri: &str) -> Self {
        let client = get_client(uri).unwrap();
        PostgresDatabase {
            uri: uri.to_string(),
...
impl BatchDatabase for PostgresDatabase {
    ...
    fn begin_batch(&self) -> Self::Batch {
        let db = PostgresDatabase::new(&self.uri);
        db.client
            .borrow_mut()
...
pub fn get_client(uri: &str) -> Result<Client, Error> {
    let mut client = Client::connect(uri, postgres::tls::NoTls)?;
    migrate(&mut client)?;
    Ok(client)
}
...
pub mod test {
    ...
    fn get_database() -> Result<PostgresDatabase, Error> {
        let default_database = PostgresDatabase::new("host=localhost user=postgres");

        let time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        let database_name = format!("test_{}", time.as_nanos());
        let statement = default_database
            .client
            .borrow_mut()
            .prepare(&format!("CREATE DATABASE {}", &database_name))?;
        default_database
            .client
            .borrow_mut()
            .execute(&statement, &[])?;

        Ok(PostgresDatabase::new(&format!(
